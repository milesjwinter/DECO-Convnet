...
def convert_images(images):
    ...
    images : numpy.ndarray
        ...
        predictions, array should have shape:(n_images, 64, 64, 3). For training,
        array should have shape:(n_images, 100, 100, 3)

    Returns
    ----------
    numpy.ndarray
        Array of normalized, grayscale images with shape:(n_images, n_rows, n_cols, 1)
    """
...
class cnn(object):
    ...
    def __init__(self, weights_file=None, model_file=None, custom_model=None, 
        ...
        weights_file : str, optional
            Path and file name of an hdf5 file containing the trained model weights 
            to be used by the CNN. (default: None)
        model_file : str, optional
            Path and file name of an hdf5 file containing a trained model. Typically,
            this should only be used when continuing an existing training session. 
            (default: None)
        custom_model : keras model, optional
            User-defined, compiled keras model to be used in place of the default
            (default: None)
        training : bool, optional
            If True, initializes the model structure used for training. If False, 
            initializes the model structure used for predictions. (default: False)
        n_classes : int
    ...
    def _build_model(self):
        ...
        if self.training:
            ...
            self.model.add(Conv2D(64, (3, 3), padding='same', input_shape=(64, 64, 1)))
    ...
    def evaluate(self, images, labels, batch_size=32, verbose=0):
        ...
        score = self.model.evaluate(images, labels, batch_size, verbose=verbose)
        return score
    ...
    def get_class_weights(self, labels):
        ...
        ----------
        labels : numpy.ndarray
            Array of labels, shape=(n_images,)

        Returns
        ----------
        dict
            dictionary of weights for each class
        """
    ...
    def get_testing_indices(self):
        """
        Get the test indices used for each fold of cross validation. To be
        called after running 'train_with_kfold()'.

        Returns
        ----------
        numpy.ndarray
            Array of arrays where each sub-array contains the testing indices 
            for a single fold. shape=(k_folds,)
        """
        return self.test_indices

    def get_training_indices(self):
        """
        Get the training indices used for each fold of cross validation. To be
        called after running 'train_with_kfold()'.

        Returns
        ----------
        numpy.ndarray
            Array of arrays where each sub-array contains the training indices 
            for a single fold. shape=(k_folds,)
        """
    ...
    def predict(self, images, batch_size=32, verbose=0):
        ...
        ----------
        images : numpy.ndarray
            Array of grayscale, normalized images to be used for class predictions. 
            Input shape = (n_image,n_row,n_cols,1)
        batch_size : int
            Batch size to use for predictions (default: 32)
        verbose : int
        ...
        ----------
        numpy.ndarray
            Array containing class probabilities for each image. The array output is
            ordered as follows: [n_images, p(worm), p(spot), p(track) , p(noise)]
            
        """
        return self.model.predict(images, batch_size=batch_size, verbose=verbose)

    def train(self, train_images, train_labels, test_images, test_labels, batch_size=32, 
              seed=None, epochs=10, initial_epoch=0, smooth_factor=None, check_point=True, 
              check_point_weights_only=True, horizontal_flip=True, vertical_flip=True,
              width_shift_range=0.08, height_shift_range=0.08, rotation_range=180.,
              zoom_range=[0.9,1.1], fill_mode="constant", cval=0, shuffle=True,
        ...
        train_images : numpy.ndarray
            Array of grayscale, normalized images to be used for training the CNN. 
            Input shape = (n_image,n_row,n_cols,1)
        train_labels : numpy.ndarray
            Array of training labels, shape=(n_images,)
        test_images : numpy.ndarray
            Array of grayscale, normalized images to be used for testing the CNN.
            Input shape = (n_image,n_row,n_cols,1)
        ...
        epochs : int
            Number of epochs to train the model. Note that in conjunction with initial_epoch, 
            the parameter epochs is to be understood as "final epoch". (default: 10)
        initial_epoch : int 
            Epoch at which to start training (useful for resuming a previous training run).
            (default: 0)
        smooth_factor : float between 0,1
            Level of smoothing to apply to one-hot label vector. Ex. smooth_factor of 0.004 
            applied to [0, 1, 0, 0], results in [0.001, 0.997, 0.001, 0.001]. (default: None)   
        check_point : bool
            If True, saves a running copy of the model corresponding to the lowest validation 
            loss epoch. Each time a new low is reached, the previous best model is over-written 
            by the new one. Model saved as 'best_checkpointed_model.h5'. (default: True)
        check_point_weights_only : bool
            If True, then only the model's weights will be saved, else the full model is saved.
            Ignored if check_point = False. (default: True)
        ...
        fill_mode : One of {"constant", "nearest", "reflect" or "wrap"}
            ...
            (default: "constant")
        cval : float or int
            Value used for interpolated pixels when fill_mode = "constant". (default: 0)
        shuffle : bool
            Whether to shuffle the order of the batches at the beginning of each epoch. 
            (default: True)
        save_model : str
            If specified, a copy of the model from the final training epoch will be saved.
            ex. save_model='my_model.h5'. Typically used for continued training (default: None)
        save_weights : str
            If specified, a copy of the model weights from the final training epoch will be saved.
            ex. save_weights='my_weights.h5'. (default: None)
        save_history : str
            If specified, the training history (accuracy and loss for training and testing) from each 
            epoch will be saved to a '.csv'. ex. save_history='my_history.csv'. (default: None)
        output_dir : str
            If specified, all model outputs will be saved to the specified directory. 
            (default: current working directory)
        """
        if not self.training:
            raise ValueError("cnn class initialized with training='False', must be 'True'")
        ...
        hist = self.model.fit_generator(datagen.flow(train_images, train_labels,
    ...
    def train_with_kfold(self, images, labels, k_folds=10, seed=None, shuffle=True,
        ...
        ----------
        images : numpy.ndarray
            Array of grayscale, normalized images to be used for training and testing 
            the CNN. shape = (n_image,n_row,n_cols,1)
        labels : numpy.ndarray
            Array of labels to be used for training and testing the CNN. shape=(n_images,)
        k_folds : int
        ...
        check_point : bool
            If True, saves a running copy of the model corresponding to the lowest validation 
            loss epoch. Each time a new low is reached, the previous best model is over-written 
            by the new one. Model saved as 'best_checkpointed_model_k.h5', where k is the current
            fold being trained. (default: True)
        check_point_weights_only : bool
            If True, then only the model's weights will be saved, else the full model is saved.
            Ignored if check_point = False. (default: True)
        horizontal_flip : bool
        ...
        save_model : str
            If specified, a copy of the model from the final training epoch will be saved.
            ex. save_model='my_model.h5' will save models as 'my_model_k.h5', where k is 
            the current fold being trained. Typically used for continued training (default: None)
        save_weights : str
            If specified, a copy of the model weights from the final training epoch will be saved.
            ex. save_weights='my_weights.h5' will save weights as 'my_weights_k.h5', where k is
            the current fold being trained. (default: None)
        save_history : str
            If specified, the training history (accuracy and loss for training and testing) from each 
            epoch will be saved to a '.csv'. ex. save_history='my_history.csv' will save history as 
            'my_history_k.csv', where k is the current fold being trained. (default: None)
        output_dir : str
            If specified, all model outputs will be saved to the specified directory. 
            (default: current working directory)
        """
        ...
        for i, indices in enumerate(skf.split(images, labels)):
            ...
            checkpointer=None
            if check_point:
                filepath = '{}/best_checkpointed_model_{}.h5'.format(output_dir,i)
                checkpointer = ModelCheckpoint(filepath, monitor='val_loss', verbose=0, 
                                               save_weights_only=check_point_weights_only,
                                               save_best_only=True, mode='auto')
            ...
            if save_model:
                ...
                self.model.save('{}/{}_{}.{}'.format(output_dir,split_str[0],i,split_str[-1]))
            if save_weights:
                split_str = save_weights.split('.')
                self.model.save_weights('{}/{}_{}.{}'.format(output_dir,split_str[0],i,split_str[-1]))
            if save_history:
                ...
                hist_vals = np.array([hist.history['acc'],
                                      ...
                                      hist.history['loss'],
                                      hist.history['val_loss']])
                np.savetxt('{}/{}_{}.{}'.format(output_dir,split_str[0],i,split_str[-1]),
                           np.transpose(hist_vals),
                           delimiter=',',
                           header='acc,val_acc,loss,val_loss')
